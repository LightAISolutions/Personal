...
```bash
ID=$(jq -r .DEPLOYMENT_ID googleAppsScripts/MasterACL/MasterACL.config.json)
...
Projects whose `DEPLOYMENT_ID` is still `YOUR_DEPLOYMENT_ID` (currently **all** of them — every project awaits its own Apps Script bootstrap) are never deployed — their workflow deploy steps exit immediately and their version bumps are repo-only bookkeeping.

## Partial OAuth Grants — the manifest is fine and the call is still denied

> **Symptom:** a Google service call fails at runtime with *"You do not have permission to call `X`. Required permissions: `<scope>`"* — the scope **is** declared in `appsscript.json`, and **no consent screen appears**. Confirmed on a prior project in this fleet (inherited history, project since removed): `SpreadsheetApp.openById` denied, which took sign-in down for every user of the app at once.
...
**Diagnosis — `diagnoseAuthorization()` (present in every project `.gs`, e.g. `MasterACL.gs`), run from the editor's Run dropdown.** It prints the effective user, the authorization status, and the scopes the grant **actually covers**, then gives a binary verdict:
...
`diagnoseOauthScopes_()` covers the other half — reading the manifest back through the Apps Script API to show what is *declared*. Run both on a permissions-shaped ACL failure, because declared-but-not-granted and genuinely-undeclared throw the identical error and only the pair separates them.
